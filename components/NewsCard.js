    ...
    return (
        <div className="flex justify-between flex-col rounded-md overflow-hidden bg-gray-200 shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300">
            <div>
                ...
                <h1 className="font-bold text-xl italic px-4 pt-3">{news.title.slice(0, 50)}</h1>
                <p className="text-sm px-4 py-2 capitalize">{news.description.slice(0, 100)}</p>
            </div>

            <div>
                <p className="p-4 text-right text-gray-500 italic text-sm">
                    {news.author} <span>- {moment.utc(news.published_at).fromNow(true).toString()} ago</span>
                </p>
                <Link href={news.url} target="_blank">
                    <button className="text-md text-center px-4 py-3 transition-all duration-200 inline-block w-full bg-red-400 hover:bg-red-500 hover:text-white">
                        Read More
                    </button>
                </Link>
            </div>
